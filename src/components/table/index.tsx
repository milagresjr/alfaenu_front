...
export function TableMain<T>({ data, columns, isLoading = false, emptyMessage = "Nenhum dado encontrado." }: TableProps<T>) {
      ...
      <thead className="bg-gray-50 dark:bg-gray-800 sticky top-0 z-10">
        <tr>
          {columns.map((col, index) => (
            <th
              key={index}
              className={`px-4 py-3 text-left text-sm font-semibold text-gray-700 dark:text-gray-300 ${col.className || ''}`}
            >
              {col.header}
            </th>
          ))}
            ...
            >
              {columns.map((col, colIndex) => {
                ...
                return (
                  <td
                    key={colIndex}
                    className={`px-4 py-3 text-sm text-gray-800 dark:text-gray-200 ${col.className || ''}`}
                  >
