...
export default function SignInForm() {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      setLoading2(false); // libera para mostrar o formulário
    }
  }, [router]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      //Alert.error("Acesso Negado",error.response.data.message);
      setLoading(false);
    }
  ...
  return (
    <div className="flex flex-col flex-1 lg:w-1/2 w-full">
      <div className="w-full max-w-md sm:pt-10 mx-auto mb-5">
        <Link
          href="/"
          className="hidden items-center text-sm text-gray-500 transition-colors hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300"
        >
          <ChevronLeftIcon />
          Back to dashboard
        </Link>
