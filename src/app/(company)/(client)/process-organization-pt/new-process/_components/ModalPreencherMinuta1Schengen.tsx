...
interface ModalPreencherMinuta1SchengenProps {
  open: boolean
...
export function ModalPreencherMinuta1Schengen({
  open,
  onOpenChange,
  onSuccess,
  cliente,
  ...
  initialValues,
}: ModalPreencherMinuta1SchengenProps) {
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.data_prevista_chegada && formData.data_prevista_saida && formData.data_prevista_saida < formData.data_prevista_chegada) {
      newErrors.data_prevista_saida = "Data de saída deve ser posterior à data de chegada"
    }
  ...
  const handleGerarConteudo = async () => {
    ...
    try {
      const response = await api.post('minuta1-schengen/gerar-pdf', payload)
      ...
      const msg = error?.response?.data?.message
        ...
        || error.message
        || "Erro ao gerar declaração de estada prevista. Tente novamente."
  ...
  const handleRegenerar = async () => {
    ...
    try {
      ...
      const msg = error?.response?.data?.message
        || error?.response?.data?.error
        || error.message
        || "Erro ao regenerar declaração de estada prevista. Tente novamente."
  ...
  const handleAprovar = () => {
    setMode('approved')
    toast.success("Declaração de estada prevista aprovada!")
  }
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      const response = await api.post('minuta1-schengen/baixar-pdf', {
        cliente_id: cliente?.id,
      ...
      link.href = url
      link.download = `declaracao_estada_prevista_${cliente?.nome?.replace(/\s/g, "_") || "documento"}.pdf`
      document.body.appendChild(link)
      link.click()
      document.body.removeChild(link)
      window.URL.revokeObjectURL(url)

      toast.success("Declaração de estada prevista gerada com sucesso!")
      onSuccess?.(url)
    } catch (error: any) {
      ...
      if (error.response && error.response.data instanceof Blob) {
        try {
          ...
          toast.error(errorJson.message || "Erro ao baixar declaração de estada prevista")
        } catch {
          toast.error("Erro ao baixar declaração de estada prevista. Tente novamente.")
        }
      } else {
        toast.error(error.message || "Erro ao baixar declaração de estada prevista. Tente novamente.")
      }
    } finally {
          ...
          <div className="p-6 overflow-y-auto">
            <DialogHeader>
              <DialogTitle className="text-2xl flex items-center gap-2">
                ...
                Declaração de Estada Prevista
              </DialogTitle>
              <DialogDescription>
                Insira os dados abaixo para gerar a declaração de estada prevista para {subtipo ? (subtipo === 'outro' ? subtipoOutroDescricao || 'Schengen' : subtipo) : 'Schengen'}.
              </DialogDescription>
            </DialogHeader>
          ...
          <div className="flex flex-col h-full max-h-[90vh]">
            <div className="shrink-0 px-6 pt-6">
              <DialogHeader>
                <DialogTitle className="text-2xl flex items-center gap-2">
                  <FileText className="h-6 w-6 text-primary" />
                  {mode === 'approved' ? 'Declaração de Estada Prevista Aprovada' : 'Pré-visualizar Declaração de Estada Prevista'}
                </DialogTitle>
                <DialogDescription>
                  {mode === 'approved'
                    ? 'A declaração de estada prevista foi aprovada. Faça o download do PDF.'
              ...
              <div className="max-w-[794px] mx-auto">
                {conteudoHtml && (
                    <iframe
                      ...
                      title="Pré-visualização da Declaração de Estada Prevista"
                    />
                  )}
              </div>
            </div>
