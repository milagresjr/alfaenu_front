...
export function ModalPreencherMinuta2({
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      const response = await api.post('minuta2/baixar-pdf', {
        cliente_id: cliente?.id,
      ...
      link.href = url
      link.download = `minuta2_${cliente?.nome?.replace(/\s/g, "_") || "documento"}.pdf`
          ...
          <div className="flex flex-col h-full max-h-[90vh]">
            <div className="shrink-0 px-6 pt-6">
              <DialogHeader>
                <DialogTitle className="text-2xl flex items-center gap-2">
                  <FileText className="h-6 w-6 text-primary" />
                  {mode === 'approved' ? 'Minuta 2 Aprovada' : 'Pré-visualizar Minuta 2'}
                </DialogTitle>
                <DialogDescription>
                  {mode === 'approved'
              ...
              <div className="max-w-[794px] mx-auto">
                {conteudoHtml && (
                    <iframe
                      ...
                      title="Pré-visualização da Minuta 2"
                    />
                  )}
              </div>
            </div>
