...
import { Loader2, Upload, FileText, AlertCircle, Trash2, Eye, Building2 } from "lucide-react"
import { toast } from "react-toastify"
import { cn } from "@/lib/utils"
import { MyClienteType } from "@/features/myClient/types"
import { useCreateSolicitacaoReservaHotel, useGetDescricaoAtivaReservaHotel } from "@/features/solicitacao-reserva-hotel/hooks/useReservaHotelQuery"

interface ModalSolicitarReservaHotelProps {
  open: boolean
...
export function ModalSolicitarReservaHotel({
  open,
  onOpenChange,
  cliente,
  onSuccess,
  dataPrevistaChegada,
  dataPrevistaSaida,
}: ModalSolicitarReservaHotelProps) {
  const [isLoading, setIsLoading] = useState(false)
  const [dataIda, setDataIda] = useState('')
  const [dataVolta, setDataVolta] = useState('')
  const [enderecoArredores, setEnderecoArredores] = useState('')
  const [observacoes, setObservacoes] = useState('')
  const [comprovativo, setComprovativo] = useState<File | null>(null)
  const [erro, setErro] = useState('')
  const fileInputRef = useRef<HTMLInputElement>(null)

  const createSolicitacaoReservaHotel = useCreateSolicitacaoReservaHotel()
  const { data: descricaoAtiva } = useGetDescricaoAtivaReservaHotel()
  ...
  const handleSubmit = async () => {
    ...
    if (!enderecoArredores) {
      setErro('O endereço/arredores é obrigatório.')
      return
    }
    ...
    try {
      ...
      formData.append('endereco_arredores', enderecoArredores)
      formData.append('comprovativo', comprovativo)
      if (observacoes) {
        formData.append('observacoes', observacoes)
      }

      createSolicitacaoReservaHotel.mutate(formData, {
        onSuccess: () => {
          toast.success('Solicitação de reserva de hotel enviada com sucesso!')
          handleClose()
          onSuccess?.()
        },
        onError: (error: Error) => {
          const err = error as unknown as { response?: { data?: { message?: string } } }
          const msg = err?.response?.data?.message || 'Erro ao enviar solicitação de reserva de hotel.'
          toast.error(msg)
          setErro(msg)
        },
        onSettled: () => {
        ...
        <div className="p-6">
          <DialogHeader>
            <DialogTitle className="text-2xl flex items-center gap-2">
              ...
              Solicitar Reserva de Hotel
            </DialogTitle>
            <DialogDescription>
              Preencha os dados abaixo para solicitar a reserva de hotel.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-6 py-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="endereco_arredores">
                Endereço / Arredores <span className="text-red-500">*</span>
              </Label>
              <textarea
                id="endereco_arredores"
                value={enderecoArredores}
                onChange={(e) => setEnderecoArredores(e.target.value)}
                rows={3}
                className={cn(
                ...
                placeholder="Endereço do hotel ou região de interesse..."
              />
            </div>
