...
import { Loader2, Upload, FileText, AlertCircle, Trash2, Eye, Plane } from "lucide-react"
import { toast } from "react-toastify"
import { cn } from "@/lib/utils"
import { MyClienteType } from "@/features/myClient/types"
import { useCreateSolicitacaoPrintVoo, useGetDescricaoAtivaPrintVoo } from "@/features/solicitacao-print-voo/hooks/usePrintVooQuery"

interface ModalSolicitarPrintVooProps {
  open: boolean
...
export function ModalSolicitarPrintVoo({
  open,
  onOpenChange,
  cliente,
  onSuccess,
  ...
  const createSolicitacaoPrintVoo = useCreateSolicitacaoPrintVoo()
  const { data: descricaoAtiva } = useGetDescricaoAtivaPrintVoo()
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formData.forEach((item, key) => {
        console.log('FormData item:', item);
        console.log('FormData key:', key);
      })

      createSolicitacaoPrintVoo.mutate(formData, {
        onSuccess: () => {
          toast.success('Solicitação de print de voo enviada com sucesso!')
          handleClose()
          onSuccess?.()
        },
        onError: (error: Error) => {
          const err = error as unknown as { response?: { data?: { message?: string } } }
          const msg = err?.response?.data?.message || 'Erro ao enviar solicitação de print de voo.'
          toast.error(msg)
          setErro(msg)
        },
        onSettled: () => {
        ...
        <div className="p-6">
          <DialogHeader>
            <DialogTitle className="text-2xl flex items-center gap-2">
              ...
              Solicitar Print de Voo
            </DialogTitle>
            <DialogDescription>
              Preencha os dados abaixo para solicitar o print de voo.
            </DialogDescription>
          </DialogHeader>
