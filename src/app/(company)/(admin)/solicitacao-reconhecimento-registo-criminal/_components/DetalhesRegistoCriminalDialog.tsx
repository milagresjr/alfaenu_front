...
import { Clock, CheckCircle, FileText, Eye, User, File, Mail, Phone, Home, MapPin, MessageSquare, Fingerprint } from "lucide-react"
import { toast } from "react-toastify"

interface DetalhesRegistoCriminalDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  solicitacao: SolicitacaoReconhecimentoRegistoCriminalType | null
}
...
export function DetalhesRegistoCriminalDialog({ open, onOpenChange, solicitacao }: DetalhesRegistoCriminalDialogProps) {
      ...
      <DialogContent className="!max-w-3xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-xl">
            <Fingerprint className="h-5 w-5 text-primary" /> Detalhes da Solicitação
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-6 py-4">
          ...
          <Section title="Dados da Solicitação" icon={<Fingerprint className="h-4 w-4" />}>
