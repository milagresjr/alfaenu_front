...
import { ReconhecimentoConsuladoType } from "@/features/solicitacao-reconhecimento-consulado/types"
import { Clock, CheckCircle, XCircle, FileText, Eye, User, File, Mail, Phone, Calendar, MessageSquare, AlertCircle, FileSignature } from "lucide-react"
import { toast } from "react-toastify"

interface DetalhesReconhecimentoConsuladoDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  solicitacao: ReconhecimentoConsuladoType | null
}
...
const statusConfig: Record<string, { label: string; color: string; icon: React.ReactNode }> = {
  ...
  nao_enviado: { label: 'Não enviado', color: 'text-gray-600 bg-gray-100 border-gray-500', icon: <AlertCircle className="h-3 w-3" /> },
  enviado: { label: 'Enviado', color: 'text-blue-600 bg-blue-100 border-blue-500', icon: <FileText className="h-3 w-3" /> },
}
...
export function DetalhesReconhecimentoConsuladoDialog({ open, onOpenChange, solicitacao }: DetalhesReconhecimentoConsuladoDialogProps) {
      ...
      <DialogContent className="!max-w-3xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-xl">
            <FileSignature className="h-5 w-5 text-primary" /> Detalhes do Reconhecimento
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-6 py-4">
          ...
          <Section title="Dados da Solicitação" icon={<FileSignature className="h-4 w-4" />}>
            <InfoRow icon={<Calendar className="h-4 w-4 text-muted-foreground" />} label="Data do Documento" value={solicitacao.data_documento ? formatarDataLong(solicitacao.data_documento) : '-'} />
