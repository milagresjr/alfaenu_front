...
import { Plane, Search, CheckCircle, XCircle, Clock, AlertCircle, FileText, Upload, Info } from "lucide-react"
...
import {
  useGetAllSolicitacoesPrintVoo,
  useAprovarSolicitacaoPrintVoo,
  useRejeitarSolicitacaoPrintVoo,
  useEnviarDocumentoPrintVoo,
} from "@/features/solicitacao-print-voo/hooks/useAdminPrintVooQuery"
import { SolicitacaoPrintVooType, SolicitacaoPrintVooStatus } from "@/features/solicitacao-print-voo/types"
import { AprovarPrintVooDialog } from "./AprovarPrintVooDialog"
import { RejeitarPrintVooDialog } from "./RejeitarPrintVooDialog"
import { EnviarPrintVooForm } from "./EnviarPrintVooForm"
import { VerMotivoRejeicaoDialog } from "./VerMotivoRejeicaoDialog"
import { VerComprovativoDialog } from "./VerComprovativoDialog"
import { DetalhesPrintVooDialog } from "./DetalhesPrintVooDialog"

type FilterType = 'todos' | SolicitacaoPrintVooStatus

const statusConfig: Record<SolicitacaoPrintVooStatus, { label: string; color: string; icon: React.ReactNode }> = {
  pendente: {
    label: 'Pendente',
...
export default function SoliPrintVooTable() {
  const [search, setSearch] = useState('')
  const [filter, setFilter] = useState<FilterType>('todos')

  const debouncedSearch = useDebounce(search, 500)

  const { data, isLoading, refetch } = useGetAllSolicitacoesPrintVoo({
    search: debouncedSearch || undefined,
    status: filter !== 'todos' ? filter : undefined,
  })

  const aprovarMutation = useAprovarSolicitacaoPrintVoo()
  const rejeitarMutation = useRejeitarSolicitacaoPrintVoo()
  const enviarDocumentoMutation = useEnviarDocumentoPrintVoo()

  const [selectedSolicitacao, setSelectedSolicitacao] = useState<SolicitacaoPrintVooType | null>(null)
  ...
  const handleDownloadDocumento = (solicitacao: SolicitacaoPrintVooType) => {
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03] p-4 min-h-[calc(100vh-130px)]">
      ...
      <TableMain
        data={solicitacoes}
        isLoading={isLoading}
        emptyMessage="Nenhuma solicitação de print de voo encontrada."
        columns={[
          {
            header: "Cliente",
            accessor: (solicitacao: SolicitacaoPrintVooType) => (
