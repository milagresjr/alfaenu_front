...
import { Clock, CheckCircle, XCircle, FileText, Eye, User, File, Mail, Phone, MapPin, Calendar, MessageSquare, Building } from "lucide-react"
import { toast } from "react-toastify"

interface DetalhesReservaHotelDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  solicitacao: SolicitacaoReservaHotelType | null
}
...
export function DetalhesReservaHotelDialog({
  open,
  onOpenChange,
  solicitacao,
}: DetalhesReservaHotelDialogProps) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="!max-w-3xl max-h-[85vh] overflow-y-auto">
        ...
        <div className="space-y-6 py-4">
          ...
          <Section title="Dados da Solicitação" icon={<Building className="h-4 w-4" />}>
            ...
            <InfoRow icon={<MapPin className="h-4 w-4 text-muted-foreground" />} label="Endereço / Arredores" value={solicitacao.endereco_arredores || '-'} />
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 border-b pb-2">
        {icon}<h3 className="text-base font-semibold">{title}</h3>
      </div>
