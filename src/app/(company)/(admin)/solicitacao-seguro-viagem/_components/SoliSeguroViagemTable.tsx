...
import {
  useGetAllSolicitacoesSeguroViagem,
  useAprovarSolicitacaoSeguroViagem,
  useRejeitarSolicitacaoSeguroViagem,
  useEnviarDocumentoSeguroViagem,
...
import { AprovarSeguroViagemDialog } from "./AprovarSeguroViagemDialog"
import { RejeitarSeguroViagemDialog } from "./RejeitarSeguroViagemDialog"
...
export default function SoliSeguroViagemTable() {
  const [search, setSearch] = useState('')
  const [filter, setFilter] = useState<FilterType>('todos')

  const debouncedSearch = useDebounce(search, 500)

  const { data, isLoading, refetch } = useGetAllSolicitacoesSeguroViagem({
    search: debouncedSearch || undefined,
    status: filter !== 'todos' ? filter : undefined,
  })

  const aprovarMutation = useAprovarSolicitacaoSeguroViagem()
  const rejeitarMutation = useRejeitarSolicitacaoSeguroViagem()
  const enviarDocumentoMutation = useEnviarDocumentoSeguroViagem()

  const [selectedSolicitacao, setSelectedSolicitacao] = useState<SolicitacaoSeguroViagemType | null>(null)
  ...
  const handleDownloadDocumento = (solicitacao: SolicitacaoSeguroViagemType) => {
    ...
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03] p-4 min-h-[calc(100vh-130px)]">
      <div className="flex justify-start items-center my-4">
        <h1 className="text-2xl text-gray-700 dark:text-gray-300 font-semibold">
          Solicitações de Seguro de Viagem
        </h1>
      </div>
