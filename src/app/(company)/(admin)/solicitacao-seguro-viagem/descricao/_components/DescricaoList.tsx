...
import {
  useGetDescricoesSeguroViagem,
  useCreateDescricaoSeguroViagem,
  useUpdateDescricaoSeguroViagem,
  useDeleteDescricaoSeguroViagem,
  useToggleDescricaoSeguroViagemActive,
} from "@/features/solicitacao-seguro-viagem/hooks/useAdminSeguroViagemQuery"
import { SolicitacaoSeguroViagemDescricaoType } from "@/features/solicitacao-seguro-viagem/types"
import { DescricaoDialog } from "./DescricaoDialog"

export default function DescricaoList() {
  const { data: descricoes, isLoading, refetch } = useGetDescricoesSeguroViagem()
  const createMutation = useCreateDescricaoSeguroViagem()
  const updateMutation = useUpdateDescricaoSeguroViagem()
  const deleteMutation = useDeleteDescricaoSeguroViagem()
  const toggleMutation = useToggleDescricaoSeguroViagemActive()

  const [openDialog, setOpenDialog] = useState(false)
  const [selectedDescricao, setSelectedDescricao] = useState<SolicitacaoSeguroViagemDescricaoType | null>(null)
  ...
  const handleEdit = (descricao: SolicitacaoSeguroViagemDescricaoType) => {
    setSelectedDescricao(descricao)
    setOpenDialog(true)
  }

  const handleDelete = async (descricao: SolicitacaoSeguroViagemDescricaoType) => {
  ...
  const handleToggle = (descricao: SolicitacaoSeguroViagemDescricaoType) => {
    ...
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03] p-4 min-h-[calc(100vh-130px)]">
      <div className="flex justify-between items-center my-4">
        <h1 className="text-2xl text-gray-700 dark:text-gray-300 font-semibold">
          Descrições de Seguro de Viagem
        </h1>
      ...
      <TableMain
        ...
        columns={[
          ...
          {
            header: "Descrição",
            accessor: (descricao: SolicitacaoSeguroViagemDescricaoType) => (
